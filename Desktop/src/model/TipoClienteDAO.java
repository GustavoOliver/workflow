...
public class TipoClienteDAO extends TipoCliente implements DAO{
	...
	public String excluir() {
		try{
			bd.getConnection();
			sql = "delete from tipo_cliente where codigo = ?";
	...
	public String gravar() {
		try{
			...
			if(localizar()){//se encontrar
				sql = "update tipo_cliente set nome=? where codigo=?";
			}
			else{
				sql = "insert into tipo_cliente values (?,?)";
	...
	public boolean localizar() {
		try{
			bd.getConnection();
			sql = "select * from tipo_cliente where codigo = ?";
			bd.st = bd.con.prepareStatement(sql);
			bd.st.setString(1, getCodigo());
	...
	public String buscarNome() {
		try{
			bd.getConnection();
			sql = "select nome from tipo_cliente where codigo = ?";
			bd.st = bd.con.prepareStatement(sql);
			bd.st.setString(1, getCodigo());
			bd.rs = bd.st.executeQuery();
			bd.rs.next();
